(() => {
    ...
    async function load() {
        ...
        $('stats-status').textContent = 'Оновлюємо дані…';
        empty($('channels'), 7, 'Завантаження…');
        empty($('publications'), 6, 'Завантаження…');
        $('totals').replaceChildren();
        $('page-info').textContent = '';
        const timeout = setTimeout(() => request.abort(), 15000);
        try {
            const response = await fetch(`${base}/health?period=${period}&offset=${offset}`, {
            ...
            data.channels.forEach(channel => {
                ...
                fields.forEach(key => {
                    const value = key === 'active' ? channel.active : channel[period][key];
                    cell(row, number(value));
                    totals[key] += value;
                });
                $('channels').append(row);
                ...
                (channel.topics || []).forEach(topic => {
                    ...
                    topicRow.className = 'stats-topic-row';
                    const topicName = cell(topicRow, topic.name);
                    if (topic.messageThreadId == null) {
                        const topicId = document.createElement('small');
                        topicId.textContent = 'Історичні записи';
            ...
            cell(total, 'Усього');
            fields.forEach(key => cell(total, number(totals[key])));
            $('totals').append(total);
            ...
            data.publications.forEach(publication => {
                ...
                platform.className = 'stats-platform';
                platform.textContent = publication.platform === 'TIKTOK' ? 'TikTok' : 'Threads';
                cell(row, '').append(platform);
                ...
                topic.textContent = publication.topicName;
                source.append(topic);
                cell(row, publication.listingId);
                cell(row, publication.adId);
                cell(row, publication.messageId);
            ...
            $('next').disabled = !data.hasMorePublications;
            ...
            $('stats-status').textContent = 'Не вдалося отримати статистику. Перевірте з’єднання та натисніть «Оновити дані».';
            empty($('channels'), 7, 'Дані недоступні');
            empty($('publications'), 6, 'Дані недоступні');
        } finally {
            clearTimeout(timeout);
            if (controller === request) $('refresh').disabled = false;
        }
    }
