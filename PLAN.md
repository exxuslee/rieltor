...
- `TIKTOK_MODE=POST` сохраняет прямую публикацию в TikTok. `TIKTOK_MODE=DRAFT` загружает фотографии в черновик TikTok
  (`MEDIA_UPLOAD`) и завершает обработку после доставки уведомления во входящие TikTok. Настройка не влияет на Threads:
  он всегда публикует сразу, если подключён.
