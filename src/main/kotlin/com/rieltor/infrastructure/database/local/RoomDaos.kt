...
internal interface CatalogDao {
    ...
    @Upsert suspend fun saveSource(row: IncomingEntity): Long
    @Query("SELECT * FROM listings WHERE id=:id") suspend fun listing(id: Long): ListingEntity?
    @Query("SELECT * FROM listings WHERE groupKey=:key") suspend fun listingForGroup(key: String): ListingEntity?
    @Query("SELECT * FROM listings ORDER BY sourceCreatedAt DESC, id DESC") suspend fun listings(): List<ListingEntity>
    @Query("SELECT COUNT(*) FROM listings") suspend fun count(): Int
    @Upsert suspend fun saveListing(row: ListingEntity): Long
    @RawQuery suspend fun query(query: androidx.room.RoomRawQuery): List<ListingEntity>
}
