...
class TikTokPublishThrottleRepositoryImpl(private val database: RoomDatabaseStore,
    private val catalog: CatalogRepository = CatalogRepository(database)) : TikTokPublishThrottleRepository {
    ...
    override fun blockUntil(blockedUntilMillis: Long) = database.settings.update { it.copy(blockedUntil = maxOf(it.blockedUntil, blockedUntilMillis)) }
    ...
    override fun trackedPublishes(nowMillis: Long, retentionMillis: Long): List<TrackedTikTokPublish> =
        catalog.listings().flatMap { catalog.publication(it, RepostDestination.TIKTOK).attempts }
            .filter { it.publishId != null && it.status in setOf("AWAITING_CONFIRMATION", "DELIVERED_DRAFT", "UNKNOWN") }
    ...
    override fun updateTrackedStatus(publishId: String, status: String, nowMillis: Long) {
        catalog.updatePublish(publishId, nowMillis) { it.copy(status = when (status) {
            "PUBLISH_COMPLETE" -> "PUBLISHED"; "FAILED" -> "FAILED"; "SEND_TO_USER_INBOX" -> "DELIVERED_DRAFT"
            else -> "AWAITING_CONFIRMATION"
        }) }
    }
    override fun removeTrackedPublish(publishId: String) = Unit // History belongs to the listing and is never erased here.
}
