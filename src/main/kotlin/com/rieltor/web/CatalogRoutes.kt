...
class CatalogQuery(private val repository: CatalogRepository, private val publicBaseUrl: String) {
    ...
    fun list(parameters: Parameters): ListingPage {
        ...
        fun add(sql: String, vararg values: Any) {
            where += sql; arguments.addAll(values)
        }

        fun values(name: String, allowed: Set<String>, column: String = name) {
            val values =
                parameters.getAll(name).orEmpty().flatMap { it.split(',') }.filter { it.isNotEmpty() }.distinct()
        ...
        val programs =
            parameters.getAll("governmentPrograms").orEmpty().flatMap { it.split(',') }.filter { it.isNotEmpty() }
                .distinct()
        require(programs.all { it in CatalogCodes.programs }) { "Invalid governmentPrograms" }
        if (programs.isNotEmpty()) add(
            "EXISTS (SELECT 1 FROM json_each(listings.governmentPrograms) WHERE value IN (${programs.joinToString { "?" }}))",
        ...
        val min = money("priceMin");
        val max = money("priceMax")
        ...
        parameters["rooms"]?.takeIf { it.isNotBlank() }?.let {
            val rooms = it.toIntOrNull(); require(rooms != null && rooms in 1..30) { "Invalid rooms" }; add(
            "rooms = ?",
            rooms
        )
        }
        ...
        val signature = sha256(parameters.entries().filter { it.key !in setOf("cursor", "limit") }.sortedBy { it.key }
        ...
        parameters["cursor"]?.let { encoded ->
            ...
            val cursor = runCatching {
                json.decodeFromString<CatalogCursor>(
            ...
            add(
                "($column ${if (ascending) ">" else "<"} ? OR ($column = ? AND id < ?))",
                cursor.value,
                cursor.value,
                cursor.id
            )
        ...
        val cursor = if (result.size > limit) shown.last().let {
            Base64.getUrlEncoder().withoutPadding().encodeToString(
                json.encodeToString(
                    CatalogCursor(
                        signature, if (sort == "newest") it.sourceCreatedAt else requireNotNull(it.price), it.id
                    )
                ).toByteArray()
            )
        } else null
...
fun Route.catalogRoutes(query: CatalogQuery) {
    get("/api/listings") {
        try {
            call.respond(query.list(call.request.queryParameters))
        } catch (error: IllegalArgumentException) {
            call.respond(HttpStatusCode.BadRequest, mapOf("error" to (error.message ?: "Invalid filters")))
        }
    }
    get("/api/listings/{id}") {
        val id = call.parameters["id"]?.toLongOrNull()
        val listing = id?.let(query::one)
        if (listing == null) call.respond(
            HttpStatusCode.NotFound, mapOf("error" to "Listing not found")
