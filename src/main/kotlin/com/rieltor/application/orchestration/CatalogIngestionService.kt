...
class CatalogIngestionService(
    ...
    private suspend fun loop(pause: Long, action: suspend () -> Unit) {
        while (currentCoroutineContext().isActive) {
            ...
            catch (error: Throwable) { logger.error("Catalog worker failed", error) }
            delay(pause)
        }
    }
    ...
    suspend fun verifyDue() {
        ...
        repository.groups().filter { group ->
            group.all { it.verifyAfter <= time && it.nextAttemptAt <= time } &&
                group.any { it.status in setOf("WAITING_STABILITY", "VERIFY_RETRY") || it.status == "PROMOTED" && (it.verifiedAt ?: 0) < time - 20 * 60_000 }
        }.forEach { rows ->
            if (verify(rows)) repository.stage(rows, if (rows.all { it.status == "PROMOTED" }) "PROMOTED" else "READY_FOR_MEDIA", now())
        }
    }
    suspend fun downloadNext(): Boolean {
        val rows = repository.groups().filter { group -> group.all {
            it.status in setOf("READY_FOR_MEDIA", "MEDIA_RETRY", "MEDIA_READY") && it.nextAttemptAt <= now()
        } }.maxWithOrNull(compareBy<List<IncomingEntity>> { it.first().sourceCreatedAt }.thenBy { it.first().id }) ?: return false
        ...
        try {
            ...
            drive.downloadCatalogPhotos(Json.decodeFromString(parsed.googleDriveUrls), config.maxCatalogPhotos, config.driveFileDelayMs,
                ...
                val cached = existing.firstOrNull { it.sourceFileId == file.id && it.sourceVersion == file.version && media.resolve(it.fileName) != null }
                val photo = cached ?: content.use {
                    ...
                    val hash = MessageDigest.getInstance("SHA-256").digest(Files.readAllBytes(path)).joinToString("") { byte -> "%02x".format(byte) }
    ...
    override fun close() { runBlocking { scope.coroutineContext[Job]?.cancelAndJoin() }; source.close() }
}
