...
class CatalogAdsParserTest {
    @Test fun `extracts exact price and programs without inventing eligibility`() {
        ...
        val result = CatalogAdsParser().parse(listOf(row), "APARTMENT 2", 1)
        assertEquals(82_001, result.price)
        assertEquals("ACTIVE", result.status)
        assertEquals("[\"EOSELIA\"]", result.governmentPrograms)
        assertEquals(64.0, result.areaM2)
        assertEquals("NEEDS_REVIEW", CatalogAdsParser().parse(listOf(row), null, 1).status)
    }
    ...
    @Test fun `takes apartment room count from configured topic type`() {
        ...
        val result = CatalogAdsParser().parse(listOf(row), "APARTMENT 3+", 1)
    ...
    @Test fun `uses other location when city is not specified`() {
        ...
        val result = CatalogAdsParser().parse(listOf(row), "APARTMENT 1", 1)
    ...
    private fun parse(text: String) = CatalogAdsParser().parse(
