...
class LandingLeadValidatorTest {
    ...
    fun `accepts a complete landing form from the public site`() {
        ...
        assertTrue(lead.fields.any { it.value == "52 м² 2 поверх" })
    }
    ...
    fun `rejects a honeypot, unexpected field, or foreign page`() {
        ...
        assertNull(validator.validate(LandingLeadSubmission("selection", validFields, "https://rieltor.dpdns.org/buy.html", "bot")))
        assertNull(validator.validate(LandingLeadSubmission("selection", validFields + ("text" to "spam"), "https://rieltor.dpdns.org/buy.html")))
        assertNull(validator.validate(LandingLeadSubmission("selection", validFields, "https://example.com/form")))
    }
