...
`listings` хранит готовые объекты продажи: `price` — общая цена объекта в целых USD,
...
Исходные сообщения остаются в `incoming_telegram_messages`; `groupKey` связывает их с объектом,
а `sourceRevision` используется для проверки актуальности перед репостом. История TikTok и Threads
остаётся в `listings`, дополнительных таблиц нет.

База создаётся Room из текущих моделей, ручных миграций нет.
...
чатов/подгрупп: объединение `TELEGRAM_MONITORED_CHAT_ID` /
`TELEGRAM_MONITORED_MESSAGE_THREAD_IDS` (или `TELEGRAM_MONITORED_TOPICS`)
и ключей `topicTypeMapping` в `settings.json`, как в основном приложении.
История чатов читается постранично, подгруппы фильтруются перед сохранением.

Используются обычные `APP_DB_PATH`, `APP_SECRETS_PATH`, `APP_PROJECT_ROOT`
и сессия `tdlib-session-id<TELEGRAM_USER_ID>`. Нужны только Telegram-секреты
`TELEGRAM_API_ID`, `TELEGRAM_API_HASH`, `TELEGRAM_USER_ID` из файла секретов
или окружения/.env. При необходимости подтвердите авторизацию Telegram;
