...
`listings.adId` — уникальный ключ объявления:
...
`senderId.userId`; миграция заполняет его у существующих сообщений из `rawMessage`.
Цена берётся после нормализации в целые USD; площади записываются без лишних нулей,
отсутствующие значения — `null`. Для отправителя-канала используется `chat-<chatId>`;
если автор старого сообщения неизвестен, используется `unknown-<chatId>:<messageId>`.
