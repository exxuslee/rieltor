...
### Ручной репост в TikTok по ID

Из корня проекта (остановив основной backend) выполните:

```powershell
.\gradlew.bat repostTikTok --args="42"
```

Linux/macOS: `./gradlew repostTikTok --args="42"`.
Для запуска из собранного JAR без Gradle:

```powershell
.\gradlew.bat shadowJar
java -cp rieltorSite-all.jar com.rieltor.tools.TikTokRepostMainKt 42
```

ID передаётся при запуске: один и тот же JAR можно использовать для разных объявлений.
Запускайте из каталога с рабочими настройками и данными; путь к JAR может быть абсолютным.
Для сборки на Windows с последующим запуском полного backend на Linux используйте
`-PtdlightNativeClassifier=linux_amd64_gnu_ssl3`, как описано в разделе развёртывания.

Аргумент — числовой **`adsTab.id`**, а не `adId` или ID сообщения Telegram.
Утилита берёт фотографии и подпись выбранного активного объявления, отправляет их только в TikTok,
сохраняет попытку и результат в `repostTab`, выводит статус и `publishId`, затем завершается.
Повторный ручной запуск создаёт новый репост, сохраняя историю. Незавершённые попытки
`PREPARED`, `SENDING`, `AWAITING_CONFIRMATION`, `UNKNOWN` блокируют повторную отправку до выяснения результата.
Отсутствующая/неактивная запись, недоступные фотографии и ошибки отправки завершают команду с ненулевым кодом.

Используются обычные `settings.json`, `secrets.json` и существующая авторизация TikTok.
Режим `tikTokMode` сохраняется: `POST` — приватный пост (`SELF_ONLY`), `DRAFT` — доставка черновика.
Ручной запуск не зависит от `tiktokEnabled`, расписания и локальной паузы `blockedUntil`
автоматической очереди. Значение `blockedUntil` при этом не сбрасывается.
Проверки незавершённых публикаций и обработка ограничений, возвращаемых API TikTok, сохраняются.
Ручная команда обновляет статусы прежних отправок, но не блокирует новую по локальному
счётчику пяти pending shares: решение о лимите принимает API TikTok.
Основной backend нужно остановить из-за общей блокировки `settings.json`.
Фотографии по ссылкам `PUBLIC_BASE_URL/media/<fileName>` раздаёт nginx напрямую из `mediaDirectory`.
Утилита передаёт эти ссылки в TikTok; HTTP-сервер и слушающий порт ей не нужны.
Telegram, Threads и фоновые обработчики при этом не запускаются.

### Каталог в SQLite

`adsTab` хранит готовые объекты продажи: `price` — общая цена объекта в целых USD,
`timestamp` — дата источника объявления (прежнее `sourceCreatedAt`). Тип сделки, исходная валюта и цена за м²/сотку
...
В версии 29 поля времени `adsTab` объединены: `sourceCreatedAt` переименовано в `timestamp` с сохранением значений; `createdAt`, `updatedAt` и `publishedAt` удалены. Каталог и API используют только `timestamp`.
