...
### Статистика каналов и посещений

Страница `docs/statistics.html` доступна по ссылке со страницы подключения сервисов.
`GET /health` возвращает JSON со статистикой `channels` (для каждого канала `day`, `month`, `total`, текущее `active`),
уникальными браузерами `visitors` и списком `publications`. Периоды календарные, в `Europe/Kyiv`.
`GET /health?period=total&offset=0` выбирает период списка репостов (`day`, `month`, `total`);
...
Миграция 29 → 30 создаёт журнал `statisticsEvents` и таблицу `siteVisitors`.
...
Сайт отправляет `POST /api/visits/{UUIDv4}` с анонимным ID из localStorage. В каждом периоде считается
число разных ID, без суммирования дневных уникальных посетителей. Посещения страницы статистики и
автоматизированного браузера не учитываются. Очистка localStorage или другое устройство дают новый ID;
это оценка числа браузеров, а не точное количество людей. Сервер не сохраняет IP и URL страницы в таблице статистики.
При последующем развёртывании следует применить обновлённый nginx allowlist для `/api/visits/…`.
