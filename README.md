...
### Статистика объявлений и каналов

Страница `docs/statistics.html` доступна по ссылке со страницы подключения сервисов.
`GET /health` возвращает JSON со статистикой `channels` (для каждого канала `day`, `week`, `total`, текущее `active`),
и списком `publications`. Периоды календарные, в `Europe/Kyiv`; неделя начинается в понедельник.
`GET /health?period=total&offset=0` выбирает период списка репостов (`day`, `week`, `total`);
список содержит до 50 записей, следующая страница — `offset=50`, признак продолжения — `hasMorePublications`.
Идентификаторы передаются строками, чтобы JavaScript не терял точность Telegram ID.

`received` — уникальные сообщения, доставленные в обработчик (альбом считается одним дописом);
`processed` — сообщения, получившие результат обработки, включая отклонённые;
`accepted` — сообщения, принятые в AdsTab; `active` — текущие активные объявления, независимо от периода.
Счётчики TikTok/Threads считают уникальные `adId` по первому подтверждённому репосту;
ожидание подтверждения и черновики не учитываются. Список содержит платформу, ID канала,
сообщения, строки AdsTab, `adId` и время первого репоста. Удалённая из каталога строка AdsTab
может остаться в истории репостов.

Статистика хранится в журнале `statisticsEvents`.
Журнал не удаляется при обычной очистке каталога. Сохранившиеся исторические записи включаются
в общие итоги; неизвестные даты получения/обработки/принятия не подменяются датой исходного сообщения.
Удалённую до миграции историю восстановить невозможно. Названия каналов можно задать полем `name`
в `monitoredTelegramChats`; пустые названия заполняются из Telegram при запуске авторизованного клиента.

API статистики, как и прежний `/health`, доступен без авторизации. Локальный frontend использует
`http://localhost:8080`; другой адрес задаётся метатегом `catalog-api`, как у каталога.

В версии 31 события статистики сохраняют `messageThreadId`. Миграция восстанавливает его по исходному сообщению или объявлению; записи без сохранившейся связи попадают в «Підгрупа невідома». В `channels[].topics` возвращаются подгруппы с названиями из `topicNames` и собственными `day`, `week`, `total`, `active`. Настроенные подгруппы показываются и при нулевых счётчиках. Суммы каналов уже включают подгруппы; повторно суммировать их не нужно. Репосты также содержат `messageThreadId` и `topicName`.

В версии 32 удалён учёт посещений: маршрут, браузерный трекер и данные посетителей. Миграция удаляет старую таблицу посещений, сохраняя журнал объявлений и репостов.
